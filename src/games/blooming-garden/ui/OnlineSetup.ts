 ...
 * definition. Either path calls `onReady` exactly once.
 */
export function renderOnlineSetup(
  ...
  wrap.className = "grdn-mode-select grdn-online-setup";
  ...
  function showError(message: string) {
    ...
    p.className = "grdn-status grdn-online-error";
  ...
  async function doHost() {
    ...
    mountWrap();
    try {
      const session = await hostOnlineRoom(hostMapId);
      if (cancelled) return;
      showWaiting(session.code, session.humanSide, session.mapId);
    } catch (err) {
  ...
  async function doJoin(rawCode: string) {
    ...
    try {
      ...
      if (cancelled) return;
      cleanup();
      onReady({ humanSide: session.humanSide, code: session.code, mapId: session.mapId });
    } catch (err) {
      showError(err instanceof Error ? err.message : "참가하지 못했습니다.");
    }
  }

  function showWaiting(code: string, humanSide: Player, mapId: string) {
    wrap.innerHTML = "";

    const codeLabel = document.createElement("p");
    codeLabel.className = "grdn-online-code-display";
    codeLabel.textContent = code;
    wrap.appendChild(codeLabel);

    const hint = document.createElement("p");
    hint.className = "grdn-tagline";
