...
/** Renders one piece shape as a small CSS grid of filled/empty squares —
 * used both for tray thumbnails (orientation 0) and the larger preview of
 * the currently selected piece's active orientation. */
export function renderShapeGrid(shape: Coord[], player: Player, cellPx: number): HTMLDivElement {
  ...
  for (let row = 0; row < height; row++) {
    for (let col = 0; col < width; col++) {
      ...
      cell.className = filled.has(`${row},${col}`)
        ? `bkd-shape-cell bkd-shape-cell--${player.toLowerCase()}`
...
export function renderPieceTray(host: HTMLElement, options: PieceTrayOptions): void {
  const { player, label, remaining, interactive, selectedPieceId, onSelectPiece } = options;

  const wrap = document.createElement("div");
  wrap.className = `bkd-tray bkd-tray--${player.toLowerCase()}`;
  ...
  for (const pieceId of remaining) {
    ...
    btn.appendChild(renderShapeGrid(PIECE_ORIENTATIONS[pieceId][0], player, 6));
