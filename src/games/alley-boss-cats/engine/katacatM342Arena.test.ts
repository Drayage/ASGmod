...
const enabled = env.RUN_KATACAT_M342_ARENA === "1";
  ...
  return {
    ...
    lossDiagnostics: {
      ...
      notRefutedExhaustiveLastDecisionLosses: rows.filter(
        (game) => !game.candidateWon && game.lastCandidateDecision?.outcome === "NOT_REFUTED_EXHAUSTIVE",
      ).length,
    },
  };
}

suite("KataCat M3.4.2 controlled arena", () => {
  let candidate: PythonCheckpointEvaluator;
  let parent: PythonCheckpointEvaluator;

  beforeAll(async () => {
    const candidateCheckpoint = env.KATACAT_M342_CANDIDATE_CHECKPOINT;
    const parentCheckpoint = env.KATACAT_M342_PARENT_CHECKPOINT;
    if (!candidateCheckpoint || !parentCheckpoint) {
      throw new Error("KATACAT_M342_CANDIDATE_CHECKPOINT and KATACAT_M342_PARENT_CHECKPOINT are required");
    }
  ...
  it("compares pre-collapse candidate against M3.4.1 and CURRENT with the reader-checked fallback fixed", async () => {
    const phase = env.KATACAT_M342_PHASE ?? "smoke";
    const gamesPerComparison = positiveInt("KATACAT_M342_GAMES_PER_COMPARISON", 32);
    ...
    const currentMs = positiveInt("KATACAT_M342_CURRENT_MS", 50);
    const maxMoves = positiveInt("KATACAT_M342_MAX_MOVES", 90);
    const captureDepth = positiveInt("KATACAT_M342_CAPTURE_DEPTH", 7);
    const finalVerifyMs = positiveInt("KATACAT_M342_FINAL_VERIFY_MS", 75);
    const finalVerifyLimit = positiveInt("KATACAT_M342_FINAL_VERIFY_LIMIT", 5);
    const rescueVerifyMs = positiveInt("KATACAT_M342_RESCUE_VERIFY_MS", 50);
    const rescueVerifyLimit = positiveInt("KATACAT_M342_RESCUE_VERIFY_LIMIT", 8);
    const rescueTotalMs = positiveInt("KATACAT_M342_RESCUE_TOTAL_MS", 450);
    const exhaustiveVerifyMs = positiveInt("KATACAT_M342_EXHAUSTIVE_VERIFY_MS", 25);
    const outputDir = resolve(env.KATACAT_M342_ARENA_OUTPUT_DIR ?? `katacat-m342-${phase}`);
    ...
    const play = async (matchup: Matchup, pairIndex: number, candidatePlayer: Player) => {
      ...
      return {
        schemaVersion: 1,
        stage: `M3.4.2_${phase.toUpperCase()}`,
        matchup,
        pairIndex,
    ...
    const parentSummary = summarize(games, "PARENT");
    ...
    const acceptance = {
      ...
      completeMirroredPairs: parentSummary.mirroredPairs.pairs === pairCount
        && currentSummary.mirroredPairs.pairs === pairCount
        && parentSummary.mirroredPairs.malformed === 0
        && currentSummary.mirroredPairs.malformed === 0,
      samePairedOpeningIds: ["PARENT", "CURRENT"].every((matchup) => {
        const rows = games.filter((game) => game.matchup === matchup);
        return new Set(rows.map((game) => game.openingId)).size === pairCount;
      }),
      legalMovesOnly,
      readerCheckedFallbackFixed: stats.CANDIDATE.decisions > 0 && stats.PARENT.decisions > 0,
    ...
    const summary = {
      schemaVersion: 1,
      stage: `M3.4.2_${phase.toUpperCase()}_ARENA`,
      phase,
      commit_sha: env.KATACAT_M342_COMMIT_SHA ?? "unknown",
      checkpoints: {
        parent: {
          path: env.KATACAT_M342_PARENT_CHECKPOINT,
          sha256: env.KATACAT_M342_PARENT_CHECKPOINT_SHA256 ?? "unknown",
        },
        candidate: {
          path: env.KATACAT_M342_CANDIDATE_CHECKPOINT,
          sha256: env.KATACAT_M342_CANDIDATE_CHECKPOINT_SHA256 ?? "unknown",
        },
      },
      options: {
        gamesPerComparison,
      ...
      parentM341: parentSummary,
      currentVeryHard: currentSummary,
      agents: stats,
      lossReplayCount: lossReplays.length,
      acceptance,
      note: "M3.4.2 changes only the checkpoint and fixes the M3.4.1 reader-checked fallback for both relative models. NOT_REFUTED_EXHAUSTIVE means the bounded reader did not find a forced capture; it is not a mathematical safety proof.",
    };
    ...
    console.log(`KATACAT_M342_ARENA:${JSON.stringify(summary)}`);
