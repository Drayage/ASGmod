...
const enabled = env.RUN_KATACAT_M36_ARENA === "1";
...
class PythonCheckpointEvaluator implements KataCatNeuralEvaluator {
  ...
  constructor(checkpoint: string, script: string) {
    this.child = spawn(env.PYTHON ?? "python", [script, `--checkpoint=${checkpoint}`], {
...
suite("KataCat M3.6 residual policy adapter arena", () => {
  let candidate: PythonCheckpointEvaluator;
  let parent: PythonCheckpointEvaluator;

  beforeAll(async () => {
    const candidateCheckpoint = env.KATACAT_M36_CANDIDATE_CHECKPOINT;
    const parentCheckpoint = env.KATACAT_M36_PARENT_CHECKPOINT;
    if (!candidateCheckpoint || !parentCheckpoint) {
      throw new Error("M3.6 candidate and parent checkpoints are required");
    }
    candidate = new PythonCheckpointEvaluator(candidateCheckpoint, "ml/katacat_m36_infer.py");
    parent = new PythonCheckpointEvaluator(parentCheckpoint, "ml/katacat_m33_infer.py");
  ...
  it("compares the bounded adapter with the frozen parent and CURRENT", async () => {
    const phase = env.KATACAT_M36_PHASE ?? "smoke";
    const gamesPerComparison = positiveInt("KATACAT_M36_GAMES_PER_COMPARISON", 32);
    if (gamesPerComparison % 2 !== 0) throw new Error("games per comparison must be even");
    const pairCount = gamesPerComparison / 2;
    const simulations = positiveInt("KATACAT_M36_SIMULATIONS", 32);
    const currentMs = positiveInt("KATACAT_M36_CURRENT_MS", 50);
    const maxMoves = positiveInt("KATACAT_M36_MAX_MOVES", 90);
    const captureDepth = positiveInt("KATACAT_M36_CAPTURE_DEPTH", 7);
    const outputDir = resolve(env.KATACAT_M36_ARENA_OUTPUT_DIR ?? `katacat-m36-${phase}`);
    const puctOptions = {
      simulations,
      cpuct: 1.35,
    ...
    const guardOptions = {
      ...
      finalVerificationMs: positiveInt("KATACAT_M36_FINAL_VERIFY_MS", 75),
      finalVerificationLimit: positiveInt("KATACAT_M36_FINAL_VERIFY_LIMIT", 5),
      rescueVerificationLimit: positiveInt("KATACAT_M36_RESCUE_VERIFY_LIMIT", 8),
      rescueVerificationMs: positiveInt("KATACAT_M36_RESCUE_VERIFY_MS", 50),
      rescueTotalMs: positiveInt("KATACAT_M36_RESCUE_TOTAL_MS", 450),
    };
    const improvedOptions = {
      verificationDepth: captureDepth,
      verificationMs: positiveInt("KATACAT_M36_EXHAUSTIVE_VERIFY_MS", 25),
    ...
    const summary = {
      schemaVersion: 1,
      stage: `M3.6_${phase.toUpperCase()}_ARENA`,
      phase,
      commit_sha: env.KATACAT_M36_COMMIT_SHA ?? "unknown",
      checkpoints: {
        parent: {
          path: env.KATACAT_M36_PARENT_CHECKPOINT,
          sha256: env.KATACAT_M36_PARENT_CHECKPOINT_SHA256 ?? "unknown",
        },
        candidate: {
          path: env.KATACAT_M36_CANDIDATE_CHECKPOINT,
          sha256: env.KATACAT_M36_CANDIDATE_CHECKPOINT_SHA256 ?? "unknown",
        },
      },
      options: {
        gamesPerComparison,
      ...
      acceptance,
      note: "M3.6 freezes M3.4.1 and adds only a bounded residual policy adapter. Both relative agents use the same bounded-reader fallback.",
    };
    ...
    console.log(`KATACAT_M36_ARENA:${JSON.stringify(summary)}`);
