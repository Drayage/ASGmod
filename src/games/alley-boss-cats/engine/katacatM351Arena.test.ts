...
const enabled = env.RUN_KATACAT_M351_ARENA === "1";
...
class PythonCheckpointEvaluator implements KataCatNeuralEvaluator {
  ...
  constructor(checkpoint: string) {
    ...
    this.child.on("exit", (code) => {
      if (code !== 0 && code !== null) {
        const error = new Error(`KataCat M3.5.1 evaluator exited ${code}: ${this.stderr}`);
        this.rejectReady(error);
        for (const pending of this.pending.splice(0)) pending.reject(error);
      }
    });
...
suite("KataCat M3.5.1 controlled arena", () => {
  let candidate: PythonCheckpointEvaluator;
  let parent: PythonCheckpointEvaluator;

  beforeAll(async () => {
    const candidateCheckpoint = env.KATACAT_M351_CANDIDATE_CHECKPOINT;
    const parentCheckpoint = env.KATACAT_M351_PARENT_CHECKPOINT;
    if (!candidateCheckpoint || !parentCheckpoint) {
      throw new Error("M3.5.1 checkpoint paths are required");
    }
  ...
  it("compares policy-head-only candidate with the fixed M3.4.1 fallback", async () => {
    const phase = env.KATACAT_M351_PHASE ?? "smoke";
    const gamesPerComparison = positiveInt("KATACAT_M351_GAMES_PER_COMPARISON", 32);
    if (gamesPerComparison % 2 !== 0) throw new Error("games per comparison must be even");
    const pairCount = gamesPerComparison / 2;
    const simulations = positiveInt("KATACAT_M351_SIMULATIONS", 32);
    const currentMs = positiveInt("KATACAT_M351_CURRENT_MS", 50);
    const maxMoves = positiveInt("KATACAT_M351_MAX_MOVES", 90);
    const captureDepth = positiveInt("KATACAT_M351_CAPTURE_DEPTH", 7);
    const outputDir = resolve(env.KATACAT_M351_ARENA_OUTPUT_DIR ?? `katacat-m351-${phase}`);
    ...
    const guardOptions = {
      ...
      finalVerificationMs: positiveInt("KATACAT_M351_FINAL_VERIFY_MS", 75),
      finalVerificationLimit: positiveInt("KATACAT_M351_FINAL_VERIFY_LIMIT", 5),
      rescueVerificationLimit: positiveInt("KATACAT_M351_RESCUE_VERIFY_LIMIT", 8),
      rescueVerificationMs: positiveInt("KATACAT_M351_RESCUE_VERIFY_MS", 50),
      rescueTotalMs: positiveInt("KATACAT_M351_RESCUE_TOTAL_MS", 450),
    };
    const improvedOptions = {
      verificationDepth: captureDepth,
      verificationMs: positiveInt("KATACAT_M351_EXHAUSTIVE_VERIFY_MS", 25),
    ...
    const play = async (matchup: Matchup, pairIndex: number, candidatePlayer: Player) => {
      ...
      return {
        schemaVersion: 1,
        stage: `M3.5.1_${phase.toUpperCase()}`,
        matchup,
        pairIndex,
    ...
    const summary = {
      schemaVersion: 1,
      stage: `M3.5.1_${phase.toUpperCase()}_ARENA`,
      phase,
      commit_sha: env.KATACAT_M351_COMMIT_SHA ?? "unknown",
      checkpoints: {
        parent: {
          path: env.KATACAT_M351_PARENT_CHECKPOINT,
          sha256: env.KATACAT_M351_PARENT_CHECKPOINT_SHA256 ?? "unknown",
        },
        candidate: {
          path: env.KATACAT_M351_CANDIDATE_CHECKPOINT,
          sha256: env.KATACAT_M351_CANDIDATE_CHECKPOINT_SHA256 ?? "unknown",
        },
      },
      options: {
        gamesPerComparison,
      ...
      acceptance,
      note: "M3.5.1 changes only the policy head and uses the fixed M3.4.1 bounded-reader fallback for both relative models.",
    };
    ...
    console.log(`KATACAT_M351_ARENA:${JSON.stringify(summary)}`);
