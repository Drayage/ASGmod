...
const enabled = env.RUN_KATACAT_M343_ARENA === "1";
...
function inferenceRequest(state: GameState) {
  ...
  return {
    ...
    lastAction: !previous || previous.type === "PASS" ? KATACAT_PASS_INDEX : previous.row * BOARD_SIZE + previous.col,
...
class PythonCheckpointEvaluator implements KataCatNeuralEvaluator {
  ...
  constructor(checkpoint: string) {
    this.child = spawn(env.PYTHON ?? "python", ["ml/katacat_m33_infer.py", `--checkpoint=${checkpoint}`], {
...
function normalizedOutcome(outcome: string): string {
  return outcome === "VERIFIED_EXHAUSTIVE_FALLBACK" ? "NOT_REFUTED_EXHAUSTIVE" : outcome;
}
function summarize(games: any[], matchup: Matchup) {
  ...
  const byCandidateSeat = Object.fromEntries(["A", "B"].map((seat) => {
    ...
    return [seat, {
      ...
      collapseLosses: seatRows.filter((game) => !game.candidateWon && game.lastCandidateDecision?.allRootActionsRefuted).length,
    }];
  }));
  ...
  let sweeps = 0, splits = 0, swept = 0, malformed = 0;
  ...
  return {
    ...
    lossDiagnostics: {
      losses,
      originalSelectionRefutedLosses: rows.filter((g) => !g.candidateWon && g.lastCandidateDecision?.selectedActionWasRefuted).length,
      allRootActionsRefutedLosses: rows.filter((g) => !g.candidateWon && g.lastCandidateDecision?.allRootActionsRefuted).length,
      notRefutedExhaustiveLastDecisionLosses: rows.filter(
        (g) => !g.candidateWon && g.lastCandidateDecision?.outcome === "NOT_REFUTED_EXHAUSTIVE",
      ).length,
    },
  };
}

suite("KataCat M3.4.3 controlled arena", () => {
  let candidate: PythonCheckpointEvaluator;
  let parent: PythonCheckpointEvaluator;
  beforeAll(async () => {
    const candidateCheckpoint = env.KATACAT_M343_CANDIDATE_CHECKPOINT;
    const parentCheckpoint = env.KATACAT_M343_PARENT_CHECKPOINT;
    if (!candidateCheckpoint || !parentCheckpoint) throw new Error("M3.4.3 checkpoint paths are required");
  ...
  afterAll(async () => { await Promise.all([candidate?.close(), parent?.close()]); });

  it("compares value-head-only candidate with the same reader-checked fallback", async () => {
    const phase = env.KATACAT_M343_PHASE ?? "smoke";
    const gamesPerComparison = positiveInt("KATACAT_M343_GAMES_PER_COMPARISON", 32);
    ...
    const currentMs = positiveInt("KATACAT_M343_CURRENT_MS", 50);
    const maxMoves = positiveInt("KATACAT_M343_MAX_MOVES", 90);
    const captureDepth = positiveInt("KATACAT_M343_CAPTURE_DEPTH", 7);
    const outputDir = resolve(env.KATACAT_M343_ARENA_OUTPUT_DIR ?? `katacat-m343-${phase}`);
    const puctOptions = {
      simulations, cpuct: 1.35, neuralPriorWeight: 0.75, scoreValueWeight: 0.05,
      tacticalShell: true, captureReadDepth: captureDepth, captureAttackMs: 25,
      captureDefenseMs: 50, captureDefenseLimit: 12,
    };
    const guardOptions = {
      finalVerificationDepth: captureDepth,
      finalVerificationMs: positiveInt("KATACAT_M343_FINAL_VERIFY_MS", 75),
      finalVerificationLimit: positiveInt("KATACAT_M343_FINAL_VERIFY_LIMIT", 5),
      rescueVerificationLimit: positiveInt("KATACAT_M343_RESCUE_VERIFY_LIMIT", 8),
      rescueVerificationMs: positiveInt("KATACAT_M343_RESCUE_VERIFY_MS", 50),
      rescueTotalMs: positiveInt("KATACAT_M343_RESCUE_TOTAL_MS", 450),
    };
    const improvedOptions = {
      verificationDepth: captureDepth,
      verificationMs: positiveInt("KATACAT_M343_EXHAUSTIVE_VERIFY_MS", 25),
      verificationLimit: 82,
    };
    const rescue = (state, player) => findBestMoveVeryHard(state, player, currentMs);
    const stats = { CANDIDATE: emptyAgentStats(), PARENT: emptyAgentStats(), CURRENT: emptyAgentStats() };
    const games: any[] = [];
    ...
    const choose = async (kind: AgentKind, state: GameState) => {
      ...
      const result = await searchKataCatPuctWithM341Fallback(
        state, evaluator, puctOptions, guardOptions, improvedOptions, rescue,
      );
      const elapsedMs = Date.now() - started;
    ...
    const play = async (matchup: Matchup, pairIndex: number, candidatePlayer: Player) => {
      ...
      while (!state.winner && state.moveHistory.length < maxMoves) {
        ...
        moves.push({
          ply: state.moveHistory.length, player: state.currentPlayer, agent: kind,
          action: decision.action, actionIndex, preStateHash, guard: compactGuard, decisionMs: decision.elapsedMs,
        });
        if (candidateTurn && compactGuard) {
          lastCandidateDecision = { ply: state.moveHistory.length, actionIndex, ...compactGuard, decisionMs: decision.elapsedMs };
        }
        state = applyAction(state, decision.action);
      }
      if (!state.winner || !state.winReason) throw new Error(`${matchup}/${pairIndex}/${candidatePlayer} did not finish`);
      ...
      return {
        schemaVersion: 1, stage: `M3.4.3_${phase.toUpperCase()}`, matchup, pairIndex,
        openingId: `paired-opening-${pairIndex}`, openingActions: opened.actions,
        candidatePlayer, winner: state.winner, winReason: state.winReason, plies: state.moveHistory.length,
        candidateWon, candidateCaptureLoss: !candidateWon && state.winReason === "CAPTURE",
        candidateTerritoryMargin: state.winReason === "TERRITORY"
          ? candidatePlayer === "A" ? marginA : -marginA : null,
        finalStateHash: kataCatStateHash(state), lastCandidateDecision, moves,
      };
    };
    ...
    for (const target of Object.values(stats)) target.meanDecisionMs = target.totalDecisionMs / Math.max(1, target.decisions);
    ...
    const acceptance = {
      ...
      legalMovesOnly,
      sameFallbackForBothRelativeModels: stats.CANDIDATE.decisions > 0 && stats.PARENT.decisions > 0,
      lossReplaysRecorded: lossReplays.length > 0,
      noRandomRollouts: true,
      passed: false,
    };
    acceptance.passed = Object.entries(acceptance).filter(([key]) => key !== "passed").every(([, value]) => value === true);
    const summary = {
      schemaVersion: 1, stage: `M3.4.3_${phase.toUpperCase()}_ARENA`, phase,
      commit_sha: env.KATACAT_M343_COMMIT_SHA ?? "unknown",
      checkpoints: {
        parent: { path: env.KATACAT_M343_PARENT_CHECKPOINT, sha256: env.KATACAT_M343_PARENT_CHECKPOINT_SHA256 ?? "unknown" },
        candidate: { path: env.KATACAT_M343_CANDIDATE_CHECKPOINT, sha256: env.KATACAT_M343_CANDIDATE_CHECKPOINT_SHA256 ?? "unknown" },
      },
      options: { gamesPerComparison, simulations, currentMs, maxMoves, captureDepth, ...guardOptions, ...improvedOptions },
      parentM341, currentVeryHard, agents: stats, lossReplayCount: lossReplays.length, acceptance,
      note: "M3.4.3 changes only the value head. Both relative models use the same M3.4.1 bounded reader fallback. NOT_REFUTED_EXHAUSTIVE is not a proof of safety.",
    };
    mkdirSync(outputDir, { recursive: true });
    writeFileSync(resolve(outputDir, "arena-summary.json"), JSON.stringify(summary, null, 2) + "\n");
    writeFileSync(resolve(outputDir, "loss-replays.jsonl"), lossReplays.map((game) => JSON.stringify(game)).join("\n") + "\n");
    writeFileSync(resolve(outputDir, "all-replays.jsonl"), games.map((game) => JSON.stringify(game)).join("\n") + "\n");
    console.log(`KATACAT_M343_ARENA:${JSON.stringify(summary)}`);
