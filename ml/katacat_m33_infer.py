...
from train_katacat_m1 import KataCatNet


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Serve KataCat M3.3 player-relative inference over newline-delimited JSON."
    )
...
def load_model(path: Path) -> KataCatNet:
    ...
    if checkpoint.get("encodingVersion") != "PLAYER_RELATIVE_V1":
        raise ValueError(
            f"Expected PLAYER_RELATIVE_V1 checkpoint, got {checkpoint.get('encodingVersion')}"
        )
...
def main() -> None:
    ...
    print(json.dumps({"ready": True, "encodingVersion": "PLAYER_RELATIVE_V1"}), flush=True)
