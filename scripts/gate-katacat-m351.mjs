...
const sameCheckpoint = training?.selected_checkpoint_sha256
  === training?.parent_checkpoint_sha256;

const technical = bool(training?.acceptance?.passed)
  && bool(regression?.passed)
  && bool(arena?.acceptance?.passed)
  && training?.selected?.frozenTacticalValidation?.regressionFailuresVsParent === 0
  && training?.acceptance?.nonPolicyParameterHashUnchanged === true
  && training?.acceptance?.frozenOutputsUnchanged === true;

if (!technical) reasons.push("technical_or_regression_failure");
if (!training?.improved_over_parent) reasons.push("selector_retained_parent_epoch_zero");

// Identical SHA results are a sanity run, not evidence of strength or regression.
if (!sameCheckpoint) {
  if (phase === "smoke") {
...
const summary = {
  schemaVersion: 1,
  stage: `M3.5.1_${phase.toUpperCase()}_GATE`,
  phase,
  passed,
  reasons,
  technical,
  sameCheckpoint,
  identicalCheckpointArenaIsSanityOnly: sameCheckpoint,
  checkpointSelection: {
    ...
    parentPolicyLoss: training.parent?.generalValidation?.policyLoss,
    selectedPolicyLoss: training.selected?.generalValidation?.policyLoss,
    parentPolicyTop1: training.parent?.generalValidation?.policyTop1,
    selectedPolicyTop1: training.selected?.generalValidation?.policyTop1,
    parentNegativeTop1Rate: training.parent?.frozenTacticalValidation?.negativeTop1Rate,
    selectedNegativeTop1Rate: training.selected?.frozenTacticalValidation?.negativeTop1Rate,
    parentMeanMargin: training.parent?.frozenTacticalValidation?.meanMargin,
    selectedMeanMargin: training.selected?.frozenTacticalValidation?.meanMargin,
  },
  frozenScope: {
    nonPolicyParameterHashUnchanged:
      training?.acceptance?.nonPolicyParameterHashUnchanged ?? null,
    frozenOutputsUnchanged:
      training?.acceptance?.frozenOutputsUnchanged ?? null,
    maxAbsDelta: training?.frozenOutputMaxAbsDelta ?? null,
  },
  regressions: {
    fallback: regression?.failures ?? null,
    tactical:
      training.selected?.frozenTacticalValidation?.regressionFailuresVsParent ?? null,
  },
  parentHeadToHead: parent,
  currentVeryHard: current,
  captureRegression: {
    evaluated: !sameCheckpoint,
    sourceM341CurrentRate: sourceCaptureRate,
    allowedMaximum: allowedCaptureRate,
    candidateRate: current.captureLossRate,
    passed: sameCheckpoint || current.captureLossRate <= allowedCaptureRate,
  },
};
