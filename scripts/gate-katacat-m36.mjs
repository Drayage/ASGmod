...
const technical = training?.acceptance?.passed === true
  ...
  && training?.acceptance?.largerHardNegativeSource === true
  && training?.trainableScope === "RESIDUAL_POLICY_ADAPTER_ONLY";

if (!technical) reasons.push("technical_or_regression_failure");
if (!training?.improved_over_parent) reasons.push("selector_retained_zero_adapter_epoch_zero");

// A zero adapter is behavior-equivalent to the parent. Its arena is sanity only.
if (!parentBehavior) {
  if (phase === "smoke") {
...
const summary = {
  schemaVersion: 1,
  stage: `M3.6_${phase.toUpperCase()}_GATE`,
  phase,
  passed,
  reasons,
  technical,
  ...
  checkpointSelection: {
    ...
    baseToCandidateKl: training.selected?.adapterValidation?.baseToCandidateKl,
    meanAbsResidualLogit: training.selected?.adapterValidation?.meanAbsResidualLogit,
    maxAbsResidualLogit: training.selected?.adapterValidation?.maxAbsResidualLogit,
  ...
  regressions: {
    ...
    tactical: training.selected?.frozenTacticalValidation?.regressionFailuresVsParent ?? null,
  },
